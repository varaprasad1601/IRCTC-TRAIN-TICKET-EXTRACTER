...
def register_user(request):
    if request.method == "POST":
        ...
        link = "http://irctctrainticketextracter.pythonanywhere.com/user/"+str(uid)
        sub = "Account Request"
        msg = "Receiving an Account Request from "+fn+"\n"+link
        to = "trainticketextracter.info@gmail.com"
        try:
        ...
        if request.method == "GET":
            ...
            if user.is_staff:
                ...
                if len(check)>0:
                    ...
                    ruser.save()
                    link = "http://irctctrainticketextracter.pythonanywhere.com"
                    sub = "IRCTC Train Ticket Extracter"
                    msg = "Greetings,\n\n𝐈𝐑𝐂𝐓𝐂 𝐓𝐫𝐚𝐢𝐧 𝐓𝐢𝐜𝐤𝐞𝐭 𝐄𝐱𝐭𝐫𝐚𝐜𝐭𝐞𝐫\n\n\nYour account has been Activated\nNow you can able to access our website by using the link:\n\n"+link
                    to = ruser.email
                    try:
                        mail = EmailMessage(sub,msg,to=[to])
                        mail.send()
            ...
            try:
                ...
                if user.is_active:
                    ...
                    try:
                        ...
                        msg = "𝐈𝐑𝐂𝐓𝐂 𝐓𝐫𝐚𝐢𝐧 𝐓𝐢𝐜𝐤𝐞𝐭 𝐄𝐱𝐭𝐫𝐚𝐜𝐭𝐞𝐫\n\nDear {}\n{} is your One Time Password (OTP)\nDo not share it with others\n\n\nThanks&Regards\nIRCTC Train Ticket Extracter".format(user.first_name,otp)
...
def extract_files(request):
    if request.user.is_authenticated:
        if "file" in request.FILES:
            ...
            remove_duplicates(list_of_details)
            remove_duplicates(list_of_details)
            ...
            context["pdf_files"] = list_of_details_length
...
def remove_duplicates(list_of_details):
    pnr_position = 7
...
def download(request):
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            filename = request.POST["file_name"]
            df1 = pd.DataFrame(list_of_details,columns=['JOURNEY DATE','FROM','TO','TRAIN NAME','TRAIN NUMBER','TIME OF DEPARTURE','TIME OF ARRIVAL','PNR NUMBER','BOOKING DATE','PASSENGERS','NUMBER OF PASSENGERS','STATUS','COACH NUMBER','SEAT NUMBER','BERTH','TOTAL TICKET FAIR','REFUND AMOUNT'])
            df2 = pd.DataFrame(unknown_files,columns=["Files"])
            print("final list :",list_of_details,unknown_files)
            with BytesIO() as b:
                writer = pd.ExcelWriter(b,engine='xlsxwriter')
                df1.to_excel(writer,sheet_name="Extracted Data",index=False,)
                df2.to_excel(writer,sheet_name="Not Extracted Files",index=False)
                writer.close()
